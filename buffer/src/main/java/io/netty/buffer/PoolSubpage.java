...
final class PoolSubpage<T> implements PoolSubpageMetric {
    ...
    PoolSubpage(PoolSubpage<T> head, PoolChunk<T> chunk, int memoryMapIdx, int runOffset, int pageSize, int elemSize) {
        ...
        this.pageSize = pageSize;
        // 为什么是16,64两个数字呢，elemSize是经过normCapacity处理的数字，最小值为16；
    ...
    void init(PoolSubpage<T> head, int elemSize) {
        ...
        if (elemSize != 0) {
            ...
            nextAvail = 0;
            bitmapLength = maxNumElems >>> 6;//long为64个byte,可用表示64个内存段的状态
            if ((maxNumElems & 63) != 0) {//如果maxNumElems不是64的整数倍,则表示段的数组长度+1
                bitmapLength ++;
            }
            // 用来表示段状态的值全部需要被清零
    ...
    long allocate() {
        ...
        // 对应位置值设置为1表示当前element已经被分配，转换成我们常见的BitSet，其实就是bitSet.set(q, true)
        bitmap[q] |= 1L << r;

        if (-- numAvail == 0) {//如果当前page没有可用的内存则从arena的pool中移除
            removeFromPool();
        }
    ...
    在初始化或重新分配时将自己加入到chunk.arena的pool中，通过arena进行后续的管理
    **/
    ...
    private int findNextAvail() {
        ...
        for (int i = 0; i < bitmapLength; i ++) {
            long bits = bitmap[i];
            if (~bits != 0) {// 说明这个位置段中还有可以分配的element
