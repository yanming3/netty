...
public class DefaultPromise<V> extends AbstractFuture<V> implements Promise<V> {
    ...
    public Promise<V> await() throws InterruptedException {
        ...
        checkDeadLock();// 检测是否产生死锁

        synchronized (this) {// 未完成则一直循环
            while (!isDone()) {
                incWaiters();// 将waiter数加1
                try {
                    /**
                     *
                     wait是在当前线程持有wait对象锁的情况下,暂时放弃锁,并让出CPU资源,并积极等待其他线程调用同一对象的notify或者notifyAll
                     方法;注意即使只有一个线程在等待,并且有其他线程调用了notify或者notifyAll方法,等待的线程只是被激活,但是它必须再次获得
                     锁才能继续往下执行.换言之,即使notify被调用,但只要锁没有被释放,原等待线程因为未获得锁仍然无法继续执行;
                     由于wait方法需要释放锁,因此它必须已经持有锁,所以wait和notify(notifyAll)都必须包裹在synchronized语句块中;
                     */
                    wait();
                } finally {
                    decWaiters(); // 将waiter数减1
                }
            }
        }
        return this;
